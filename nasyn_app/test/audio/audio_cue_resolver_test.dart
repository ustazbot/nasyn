...
void main() {
  ...
  group('Surah selepas Fatihah (FIQH RULE — qiyam, Full Recite)', () {
    test(
      'rakaat 1 → Fatihah + surahRakaat1; rakaat 2 → Fatihah + surahRakaat2',
      () {
        expect(
          resolver.resolve(
            PrayerState.qiyam,
  ...
  group('needsTakbirTransition', () {
    test('is false for takbiratulIhram, iktidal, salam, selesai', () {
      expect(
        resolver.needsTakbirTransition(PrayerState.takbiratulIhram),
        isFalse,
      );
    ...
    test('is true for every other state', () {
      ...
      for (final state in otherStates) {
        expect(
          resolver.needsTakbirTransition(state),
          isTrue,
          reason: '$state should get a takbir prefix',
        );
      }
    });
  });
  ...
  test(
    'semua fail Fatihah & surah pendek wujud dalam assets (kunci drift nama fail bila tukar qari)',
    () {
  ...
  test(
    'never resolves to a not-yet-recorded asset for any state/level combination',
    () {
      for (final state in PrayerState.values) {
        for (final level in AssistanceLevel.values) {
          for (final path in resolver.resolve(
            state,
            level,
            zuhurConfig,
            surahRakaat1: NasynAudio.alKafirun,
            ...
            expect(
              ...
              isFalse,
              reason:
                  '$state/$level resolved to a not-yet-recorded asset: $path',
            );
          }
        }
      }
